...
5. **自包含 / 松耦合**：技能应能独立工作，正文**不要点名依赖其它技能**（subagent 场景对方可能不在，且改名会留悬空引用）。跨技能协作靠 agent 按各自 `description` 路由，不靠正文互相点名。需要区分场景时，把**边界写进 `description`**、用**能力描述**而非技能名（如"若有记忆类技能"而非某个具体名字）。同一 repo 内自控的一组技能点名可接受，但能力描述更稳。
